...
class CreatePatientRequest extends \App\Http\Requests\Api\BaseRequest
{
    ...
    {
        $this->replace($this->only(['first_name', 'last_name', 'uhid', 'facility_code', 'gender', 'dob', 'mobile_isd_code', 'mobile_no', 'email', 'city_name', 'state_name', 'country_name', 'pin_code', 'aadhar_number', 'passport_number', 'pan_number', 'emergency_contact_name', 'emergency_contact_no']));
    ...
    public function rules()
    {
        return [
            ...
            'last_name' => 'nullable|min:1|alpha',
            'uhid' => 'required_with:facility_code',
            'facility_code' => 'required_with:uhid',
            ...
            'mobile_isd_code' => 'required|exists:countries,isd_code',
            'mobile_no' => 'required|regex:/^[1-9]{1,1}?[0-9]{9,9}$/',
            ...
            'emergency_contact_name' => 'nullable|min:1|regex:/^[a-zA-Z ]*$/',
    ...
    public function messages()
    {
        ...
        return [
            ...
            'mobile_isd_code.required' => trans('errors.PATIENT_140'),
            'mobile_isd_code.exists' => trans('errors.PATIENT_141'),

            'mobile_no.required' => trans('errors.PATIENT_108'),
            'mobile_no.regex' => trans('errors.PATIENT_131'),
            ...
            'emergency_contact_name.regex' => trans('errors.PATIENT_129'),
    ...
    public function validate()
    {
        ...
        if (!empty($this->country_name)) {
            ...
            if (strtolower($this->mobile_isd_code) != strtolower($country->isd_code)) {
                abort(400, trans('errors.PATIENT_141'), es('mobile_isd_code'));
            }
        }
        ...
        if (!empty($this->state_name)) {
            ...
            if (strtolower($this->mobile_isd_code) != strtolower($state->country->isd_code)) {
                abort(400, trans('errors.PATIENT_141'), es('mobile_isd_code'));
            }
        ...
        if (!empty($this->city_name)) {
            ...
            if (strtolower($this->mobile_isd_code) != strtolower($city->state->country->isd_code)) {
                abort(400, trans('errors.PATIENT_141'), es('mobile_isd_code'));
            }
        ...
        $this->request->set('isd_code', $this->mobile_isd_code);
        $this->offsetUnset('mobile_isd_code');
        ...
        $this->request->set('is_active', 1);
        $this->request->set('is_verified', 1);
        $this->request->set('create_source', config('constants.CREATION_SOURCE_INTERNAL')['aggregator_'.strtolower($this->header('aggregatorType'))]);
    }
}
